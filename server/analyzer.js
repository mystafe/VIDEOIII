...
async function uploadFileToGemini(filePath, mimeType, onProgressUpdate) {
  ...
  try {
    ...
    send({ type: 'error', message: `Gemini File Upload Error: ${errorMessage}` });
    return null;
  }
}
...
async function analyzeVideoInBatchesGemini(videoPath, settings, onProgressUpdate) {
  ...
  const { send, uiTexts } = onProgressUpdate;
  if (!genAI) {
    send({ type: 'error', message: 'Gemini API key not configured.' });
    return;
  }
  ...
  for (let currentBatch = 0; currentBatch < totalBatches; currentBatch++) {
    ...
    if (currentBatch === 0) {
      promptText = analysisType === 'meeting'
  ...
  if (processedBatches === totalBatches) {
    ...
    send({ type: 'error', message: 'Video analysis could not be completed.' });
  }
...
async function analyzeUploadedMediaGemini(framePaths, audioPath, settings, onProgressUpdate) {
  ...
  let audioFile = null;
  if (audioPath) {
    const audioMimeType = path.extname(audioPath).toLowerCase() === '.mp3' ? 'audio/mp3' : 'audio/webm';
    audioFile = await uploadFileToGemini(audioPath, audioMimeType, onProgressUpdate);
  ...
  let promptText = analysisType === 'meeting'
      ...
      : `${languageInstruction}\n\nYou are an expert video interpretation AI. Describe in detail the content of the video based on the provided frames and audio.`;
...
async function analyzeVideoInBatchesOpenAI(videoPath, settings, onProgressUpdate) {
  ...
  let systemPrompt = analysisType === 'meeting'
  ...
  systemPrompt += `\n**ULTIMATE RULE: YOUR ENTIRE RESPONSE MUST BE EXCLUSIVELY IN THE FOLLOWING LANGUAGE: "${outputLanguage}". DO NOT DEVIATE.**`
  ...
  for (let currentBatch = 0; currentBatch < totalBatches; currentBatch++) {
    ...
    await new Promise((resolve, reject) => {
      ffmpeg(videoPath).inputOptions([`-ss ${startTimeSeconds}`]).outputOptions([`-t ${secondsPerBatch}`, `-vf fps=1/${frameInterval},scale=${config.FRAME_WIDTH}:-1`, `-qscale:v ${config.FRAME_QUALITY}`]).output(framePattern).noAudio().on('end', resolve).on('error', reject).run();
    }).catch(err => { send({ type: 'error', message: `Frame extraction failed: ${err.message}` }); throw err; });

    let transcript = "No audio detected in this segment.";
    let audioExtractionFailed = false;
    await new Promise((resolve) => {
      ffmpeg(videoPath).inputOptions([`-ss ${startTimeSeconds}`]).outputOptions([`-t ${secondsPerBatch}`]).output(audioChunkPath).noVideo().audioCodec('libmp3lame').on('end', resolve).on('error', () => { audioExtractionFailed = true; resolve(); }).run();
    });

    if (!audioExtractionFailed) {
      try {
        const tr = await openai.audio.transcriptions.create({ file: createReadStream(audioChunkPath), model: 'whisper-1' });
...
async function analyzeUploadedMediaOpenAI(framePaths, audioPath, settings, onProgressUpdate) {
  const { outputLanguage, analysisType } = settings;
  const { send, uiTexts } = onProgressUpdate;
  if (!OPENAI_API_KEY) { send({ type: 'error', message: 'OPENAI_API_KEY not configured.' }); return; }
  ...
  if (audioPath) {
    try {
      const tr = await openai.audio.transcriptions.create({ file: createReadStream(audioPath), model: 'whisper-1' });
      transcript = tr.text.trim();
    } catch (error) {
      transcript = `Audio transcription failed: ${error.message}`;
      send({ type: 'status', message: transcript });
    }
  }
  ...
  for (const fp of framePaths) {
    ...
    if (base64Image) { imageParts.push({ type: 'image_url', image_url: { url: base64Image, detail: "low" } }); }
  }
  
  let userPromptText = analysisType === 'meeting'
      ? `You are an expert meeting analyst. Based on the following transcript and images, generate a complete report including a full transcript, key decisions made, and action items.`
      : `You are an expert video interpretation AI. Using the provided images and audio transcript, describe in detail everything that is happening in the video.`;
      
  userPromptText = `**ULTIMATE RULE: YOUR ENTIRE RESPONSE MUST BE EXCLUSIVELY IN THE FOLLOWING LANGUAGE: "${outputLanguage}".**\n\n${userPromptText}\n\nAudio Transcript: "${transcript}"`;
...
async function analyzeVideoInBatches(videoPath, settings, onProgressUpdate) {
  const handler = settings.aiModule === 'openai' ? analyzeVideoInBatchesOpenAI : analyzeVideoInBatchesGemini;
  return handler(videoPath, settings, onProgressUpdate);
}

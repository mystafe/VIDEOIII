...
// FFmpeg örneğini component dışında bir kere oluşturarak her render'da yeniden yaratılmasını engelle.
const ffmpeg = createFFmpeg({
  log: true, // Geliştirme sırasında konsolda detaylı logları görmek için 'true' yap.
  // Çekirdek (core) dosyasını harici bir CDN'den yükleyerek paket boyutunu küçük tut.
  corePath: "https://unpkg.com/@ffmpeg/core@0.10.0/dist/ffmpeg-core.js",
});

const Spinner = () => <div className="spinner"></div>;
// Sunucu adresini buradan yönet
...
function App() {
  ...
  const handleUpload = async () => {
    ...
    if (useServer) {
      ...
      try {
        ...
        if (frames.length === 0) {
            throw new Error("FFmpeg did not extract any frames. The video format might be unsupported by the browser codec.");
        }

        addLog(`On-device processing finished: Extracted ${frames.length} frames and an audio blob. Now uploading...`);
        setUploadProgress(0);
        ...
        addLog('Upload successful. Server is now processing the analysis.');
  ...
  const processVideoWithWASM = async (file, interval, maxDuration, onProgress) => {
    try {
        if (!ffmpeg.isLoaded()) {
            onProgress(0, 'Loading FFmpeg Core (first use may be slow)...');
            await ffmpeg.load();
        }
        ...
        try {
          await ffmpeg.run('-i', inputFileName, '-t', String(maxDuration), '-q:a', '0', '-map', 'a', audioOutputFileName);
        } catch(e) {
            onProgress(35, 'No audio stream found or could not be extracted.');
        ...
        let audioBlob = null;
        try {
            const audioData = ffmpeg.FS('readFile', audioOutputFileName);
            audioBlob = new Blob([audioData.buffer], { type: 'audio/mp3' });
        ...
        while (true) {
            try {
                const frameData = ffmpeg.FS('readFile', `frame-${i}.jpg`);
                frames.push(new Blob([frameData.buffer], { type: 'image/jpeg' }));
                ffmpeg.FS('unlink', `frame-${i}.jpg`);
                i++;
            } catch (e) { break; } 
        }
        
        ffmpeg.FS('unlink', inputFileName);
        if (audioBlob) ffmpeg.FS('unlink', audioOutputFileName);
        
        onProgress(100, 'On-device processing finished.');
        return { audioBlob, frames };
    } catch(error) {
        console.error("An error occurred during FFmpeg processing in the browser:", error);
        throw new Error("FFmpeg processing failed. The video file may be corrupted or in an unsupported format.");
    }
  };
